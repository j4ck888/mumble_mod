...
public:
	...
	SearchResultItem(const SearchResult &result, unsigned int id, parent_t parent,
		...
		const qsizetype begin  = result.begin;
		const qsizetype length = result.length;
#else
		const auto begin  = static_cast< int >(result.begin);
		const auto length = static_cast< int >(result.length);
#endif
